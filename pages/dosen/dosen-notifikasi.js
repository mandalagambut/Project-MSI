...
// ========== LOAD NOTIFIKASI ==========
function loadNotifikasiDosen() {
    notifikasiData = JSON.parse(localStorage.getItem('notifikasiDosen')) || [];
        ...
        notifikasiData = [
            {
                ...
                pesan: 'Mahasiswa A mengajukan konsultasi "📝 Skripsi" untuk tanggal 20 Juni 2026.',
                tanggal: new Date(Date.now() - 3600000).toISOString(),
                dibaca: false,
                link: 'kelola-pengajuan.html',
                icon: '📝'
            },
            {
                id: 'N-2',
                judul: '📝 Pengajuan Baru',
                pesan: 'Mahasiswa B mengajukan konsultasi "💻 Media Pembelajaran" untuk tanggal 22 Juni 2026.',
            ...
            },
            {
                ...
                pesan: 'Anda telah menyetujui pengajuan konsultasi "🏢 Magang" dari Mahasiswa C.',
            ...
            },
            {
                ...
                pesan: 'Anda memiliki jadwal konsultasi dengan Mahasiswa A hari ini pukul 09:00 WIB.',
                tanggal: new Date(Date.now() - 172800000).toISOString(),
                dibaca: true,
                link: 'jadwal-konsultasi.html',
                icon: '📅'
            }
        ];
        localStorage.setItem('notifikasiDosen', JSON.stringify(notifikasiData));
    }

    renderNotifikasiDosen();
    updateBadgeDosen();
}

// ========== RENDER NOTIFIKASI ==========
function renderNotifikasiDosen() {
    const container = document.getElementById('notifikasiListDosen');
    ...
    let html = '';
    notifikasiData.forEach((item) => {
        const isRead = item.dibaca;
        const timeAgo = getTimeAgo(item.tanggal);
        const icon = item.icon || '🔔';

        html += `
            <div class="notifikasi-item ${isRead ? 'read' : 'unread'}" onclick="detailNotifikasiDosen('${item.id}')">
                ...
                <button class="notifikasi-action" onclick="event.stopPropagation(); deleteNotifikasiDosen('${item.id}')">✕</button>
            </div>
        `;
    });
...
// ========== DETAIL NOTIFIKASI ==========
function detailNotifikasiDosen(id) {
...
// ========== DELETE NOTIFIKASI ==========
function deleteNotifikasiDosen(id) {
...
// ========== CLEAR ALL ==========
function clearAllNotificationsDosen() {
    if (notifikasiData.length === 0) {
        showNotification('ℹ️ Tidak ada notifikasi', 'info');
        return;
    }
